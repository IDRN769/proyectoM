...
class UsuariosModel extends Query
{
    ...
    public function modificarUsuario(string $usuario, string $nombre, string $clave, int $id_caja, int $id)
    {
        $this->usuario = $usuario;
        $this->nombre = $nombre;
        $this->clave = $clave;
        ...
        $datos = array($this->usuario, $this->nombre,$this->clave, $this->id_caja, $this->id);
        $data = $this->save($sql, $datos);
        if ($data == 1) {
            $res = "modificado";
        } else {
    ...
    public function eliminarPerUser(int $id)
    {
        $this->id = $id;
